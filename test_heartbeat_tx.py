...
OPERATOR_ADDRESSES = "0x8a6d025f1102E2768bF60381711CaBe9299fbA80".lower()
...
def main():
    ...
    if input_data.startswith(HEARTBEAT_SIG):
        ...
        blk = w3.eth.get_block(
            tx.blockNumber if hasattr(tx, "blockNumber") else tx["blockNumber"],
        ...
        if str(tx_from).lower() == OPERATOR_ADDRESSES.lower() and tx_input.startswith(
            "0x3defb962"
        ):
            print(
                f"-> Heartbeat tx found in block {blknumber} hash 0x{blkhash} and timestamp {ts}"
            )
    else:
